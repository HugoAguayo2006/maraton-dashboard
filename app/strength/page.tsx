...
export default async function StrengthPage() {
  ...
  return (
    <>
      ...
      <section className="mb-5 grid grid-cols-2 gap-2.5 min-[480px]:grid-cols-3 sm:gap-3">
        <Stat label="Rutinas" value={String(routines.length)} />
        <Stat label="Sesiones" value={String(history.length)} />
        <div className="col-span-2 min-[480px]:col-span-1"><Stat label="Volumen" value={`${formatStrengthValue(convertKilograms(volumeKg, unit), 0)} ${unit}`} /></div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(300px,.7fr)]">
        <section className="min-w-0">
          <div className="mb-3 flex min-w-0 items-center justify-between gap-3"><div className="min-w-0"><p className="eyebrow">Tus rutinas</p><h2 className="mt-2 text-xl font-bold">Listas para entrenar</h2></div><Link href="/strength/routines/new" className="flex min-h-11 shrink-0 items-center gap-1.5 rounded-xl bg-white px-3 text-[11px] font-bold text-accent shadow-sm"><Plus size={15} /> Nueva</Link></div>
