...
export function DailyInspirationCard({ inspiration }: { inspiration: DailyInspiration }) {
  ...
  return (
    <aside className="min-w-0 rounded-[24px] border border-line/80 bg-white/80 px-4 py-4 shadow-[0_8px_30px_rgba(17,17,20,.045)] backdrop-blur-sm sm:px-5">
      <div className="flex items-start gap-3">
        <span className="grid size-9 shrink-0 place-items-center rounded-2xl bg-accent-soft text-accent">
          <Icon size={17} />
        </span>
        <div className="min-w-0">
          <p className="break-words text-xs leading-5 font-semibold text-ink sm:text-[13px]">
            “{inspiration.text}”
          </p>
